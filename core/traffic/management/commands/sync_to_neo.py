...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Starting sync from record #{start_from} with batch size {batch_size}..."
        )
        ...
        all_logs = TrafficLogs.objects.all()[start_from:].iterator(
            ...
            for log in all_logs:
                ...
                if len(batch) >= batch_size:
                    ...
                    if count % (batch_size * 10) == 0:
                        self.stdout.write(
                            f"{count} records processed, {new_count} new records synced..."
                        )
